%% Tomographic Reconstruction using gradient projections
% Based on the work of: Piotr Makowski, Wojciech Krauze, Paweł Ossowski
% Author: Julianna Winnik
...
%% Ensure compatibility with Gradient Optical Diffraction Tomography (GODT)

if ~exist('SINOamp', 'var')
    SINOamp = zeros(size(SINOph)); 
    warning('Shearing amplitude not measured; assuming uniform amplitude, which results in a zero derivative.');
else
    warning('It is assumed that SINOamp contains (amp - shifted_amp) / amp.');
end
...
% Enforce supported approximation mode
if ~exist('Approx', 'var') || ~strcmp(Approx, 'GradRytov')
    Approx = 'GradRytov';
    warning('Only "GradRytov" mode is supported when input data comes from shearing interferometry. Forcing Approx = "GradRytov".');
end
